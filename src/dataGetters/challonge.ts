...
const getEventDataImpl: () => T.H2HBuilder<T.H2HEvent> = $.X(function* () {
  ...
  if (
    ...
    if (cached) {
      return yield* this.trying(
        () => $.pure(T.H2HEvent.parse(cached)),
        (e) => $.Err(T.H2HError.ParseCached(e)),
      );
    }
  }
  ...
  let nameRes: $.Result<string, T.H2HError> = $.Err(
    T.H2HError.MissingData("event.name"),
  );
  let dateRes: $.Result<number, T.H2HError> = $.Err(
    T.H2HError.MissingData("event.date"),
  );
  ...
  const baseSets: Record<
    string | number,
    Pick<T.H2HSet, "id" | "slots" | "winnerId">
  > = {};
  for (const bracketEl of bracketEls) {
    ...
    for (const matchEl of matchEls) {
      ...
      const set: (typeof baseSets)[string] = {
        id: setId,
        slots: [],
        winnerId: null,
      };
  ...
  let lastSet: null | T.H2HSet = null;
  let wasGrands = false;
  let isLosers = false;

  const slotEntrantId = (slot?: T.H2HSlot) => slot?.entrant?.id || "";

  function slotsKey(set?: Partial<T.H2HSet> | null): string {
    if (!set) {
      return "";
    }
  ...
  const gfEntrants = new Set<string | undefined | number>();
  const nonGfEntrants = new Set<string | undefined | number>();
  ...
  const sets = $.mapValues(baseSets, (baseSet) => {
    ...
    const slotName = (slot: T.H2HSlot) =>
      entrants[slotEntrantId(slot)]?.participants[0]?.name;
  ...
  function roundNum(set: T.H2HSet) {
    if (set.isGrands) {
      return 0;
    }
    if (set.isLosers) {
  ...
  function roundLabel(set: T.H2HSet) {
    if (set.isGrands) {
